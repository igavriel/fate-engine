...
test.describe("Facelift smoke", () => {
  test("themed flow: seal -> vessels -> bind -> shrine (background, headline, combat, aftermath)", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/you slip away|the omen breaks|the shrine claims you/i)
    ).toBeVisible();
