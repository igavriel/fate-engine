...
- **Paths:** `src/ui/theme/**`, `src/ui/components/**`, `src/ui/motion/**`, and `src/ui/nav/**`.
...
- **Tests:** Theme unit tests live next to source under `src/ui/theme/*.test.ts` and `src/ui/components/*.test.tsx`. Integration tests for hub copy live in `tests/integration/ui/hub-copy.test.ts`. Nav unit tests live in `src/ui/nav/routes.test.ts`; routing integration tests in `tests/integration/ui/routing.test.ts`.
