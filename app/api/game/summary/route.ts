...
import { getSummary, CombatError } from "@/server/game/combatService";
...
async function getSummaryHandler(request: Request) {
  ...
  try {
    const result = await getSummary(userId, slotIndex);
...
export const GET = withRequestLogging(getSummaryHandler);
