...
function CombatPageInner() {
  ...
  const fetchCombat = useCallback(async () => {
    ...
    if (!res.ok) {
      ...
      if (err.error?.code === "NO_ACTIVE_ENCOUNTER") {
        router.replace(`${routes.shrine(Number(slotNum))}&message=no_encounter`);
        return null;
      }
  ...
  async function handleAction(type: "ATTACK" | "HEAL" | "RETREAT") {
    ...
    try {
      ...
      if (outcome === "CONTINUE") {
        ...
        router.push(routes.shrine(Number(slotNum)));
      }
    } catch {
  ...
  if (error && !combat) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-12">
        ...
        <Link
          href={routes.shrine(Number(slotNum))}
          className="mt-4 inline-flex min-h-[44px] items-center text-sm text-zinc-400 hover:text-zinc-200"
        >
          ← Back to Hub
        </Link>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8" data-testid="combat">
      <h1 className="text-2xl font-bold text-zinc-100">{screenCopy.combat}</h1>
      <p className="text-zinc-400">
        {labels.Slot} {combat.slotIndex}
      </p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className={panel()}>
          <h2 className="text-sm font-medium text-amber-500/90 uppercase">
            THE {labels.Enemy.toUpperCase()}
          </h2>
...
export default function ShrineCombatPage() {
  return (
    <Suspense
      fallback={
