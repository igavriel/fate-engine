...
import { panel, buttonGhost } from "@/src/ui/theme/classnames";
...
function ShrinePageInner() {
  ...
  async function handleStartEncounter(choiceId: string) {
    ...
    try {
      ...
      if (res.ok) {
        router.push(routes.combat(Number(slotNum)));
        return;
      }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8" data-testid="shrine">
      {debugOn && status && (
      ...
      {encounterActive && (
        <div className="mb-4 flex items-center gap-3">
          ...
          <Link
            href={routes.combat(Number(slotNum))}
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs uppercase text-zinc-500">{item.catalog.itemType}</p>
                      <div className="mt-1 flex flex-wrap gap-3 text-sm text-zinc-400">
                        ...
                        <span>
                          Sell: {item.catalog.sellValueCoins * item.quantity}{" "}
                        ...
                        <>
                          {!isEquipped ? (
                            (() => {
                              ...
                              const belowLevel = reqLevel > 1 && run.level < reqLevel;
                              return (
                                <button
                                  type="button"
