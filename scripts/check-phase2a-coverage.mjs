#!/usr/bin/env node
/**
 * Phase 2A coverage gate: fails if line coverage for content-pack files is below 90%.
...
function main() {
  if (!fs.existsSync(COVERAGE_FILE)) {
    console.error("check-phase2a-coverage: coverage file not found. Run: pnpm test");
    process.exit(1);
  }
  ...
  for (const relPath of PHASE2A_FILES) {
    const absPath = path.resolve(cwd, relPath);
    const key =
      data[absPath] !== undefined
        ? absPath
        : Object.keys(data).find(
            (k) => k.includes("domain/enemies") && k.endsWith(path.basename(relPath))
          );
    const fileCoverage = key ? data[key] : null;

    if (!fileCoverage) {
      console.error(`check-phase2a-coverage: no coverage data for ${relPath}`);
      failed = true;
      continue;
    }
  ...
  if (failed) {
    console.error("check-phase2a-coverage: Phase 2A coverage gate failed.");
    process.exit(1);
  }
  console.log("check-phase2a-coverage: Phase 2A coverage gate passed.");
}
