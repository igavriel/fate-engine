...
import { POST } from "@/app/api/game/equip/route";
...
vi.mock("@/server/game/inventoryService", () => ({
  equipItem: (userId: string, slotIndex: number, equipmentSlot: string, inventoryItemId: string) =>
    mockEquipItem(userId, slotIndex, equipmentSlot, inventoryItemId),
...
const sampleStatus = {
  ...
  run: {
    ...
    effectiveStats: { attack: 7, defense: 5, luck: 5, hpMax: 20 },
    equipped: { weapon: "item-1", armor: null },
    lastOutcome: "NONE",
  },
};
const sampleInventory: unknown[] = [];

describe("POST /api/game/equip", () => {
  beforeEach(() => {
    mockRequireAuth.mockReset();
    mockParseJson.mockReset();
    mockEquipItem.mockReset();
  ...
  it("returns GameError status when equipItem throws GameError", async () => {
    mockRequireAuth.mockResolvedValue("user-1");
    ...
    mockEquipItem.mockRejectedValue(new GameError("SLOT_EMPTY", "Slot empty", 400));
    const res = await POST(
      new Request("http://localhost/api/game/equip", {
        method: "POST",
        headers: { "Content-Type": "application/json", Cookie: "fe_auth=x" },
  ...
  it("returns 404 when ITEM_NOT_FOUND", async () => {
    ...
    mockEquipItem.mockRejectedValue(new Error("ITEM_NOT_FOUND"));
    const res = await POST(
      new Request("http://localhost/api/game/equip", {
        method: "POST",
        headers: { "Content-Type": "application/json", Cookie: "fe_auth=x" },
  ...
  it("returns 404 when EQUIPMENT_NOT_FOUND", async () => {
    ...
    mockEquipItem.mockRejectedValue(new Error("EQUIPMENT_NOT_FOUND"));
    const res = await POST(
      new Request("http://localhost/api/game/equip", {
        method: "POST",
        headers: { "Content-Type": "application/json", Cookie: "fe_auth=x" },
  ...
  it("returns 500 on other errors", async () => {
    ...
    mockEquipItem.mockRejectedValue(new Error("DB_ERROR"));
    const res = await POST(
      new Request("http://localhost/api/game/equip", {
        method: "POST",
        headers: { "Content-Type": "application/json", Cookie: "fe_auth=x" },
  ...
  it("returns 200 with status and inventory on success", async () => {
    ...
    mockEquipItem.mockResolvedValue(undefined);
