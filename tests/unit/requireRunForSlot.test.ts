...
/** Mock tx that delegates saveSlot.findUnique to the same mock as prisma (for requireRunForSlotWithTx). */
function mockTx() {
  return {
    saveSlot: { findUnique: (...args: unknown[]) => mockFindUnique(...args) },
  };
}
...
describe("requireRunForSlotWithTx", () => {
  const userId = "user-1";
  const slotIndex = 1;
  ...
  it("when slot missing → throws GameError SLOT_NOT_FOUND", async () => {
    ...
    await expect(
      requireRunForSlotWithTx(mockTx() as never, userId, slotIndex)
  ...
  it("when valid → returns slot, character, run", async () => {
    ...
    const result = await requireRunForSlotWithTx(mockTx() as never, userId, slotIndex);
