...
describe("applyPotion", () => {
  it("caps heal at hpMax", () => {
    ...
    const { newHp, remainingQuantity } = applyPotion(18, hpMax, healPercent, 1);
  ...
  it("heals by floor(hpMax * healPercent/100)", () => {
    ...
    const { newHp } = applyPotion(50, hpMax, healPercent, 1);
    expect(newHp).toBe(75);
  });

  it("decrements quantity", () => {
    const { remainingQuantity } = applyPotion(10, 20, 25, 3);
    expect(remainingQuantity).toBe(2);
  });

  it("returns same hp and 0 remaining when quantity is 0", () => {
    const { newHp, remainingQuantity } = applyPotion(10, 20, 25, 0);
